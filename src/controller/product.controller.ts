    ...
    async create(req:req , res:res){
        try{
            ...
            if(Array.isArray(req.files)){
                images = req.files.map((file:Express.Multer.File)=>`https://shophubapi.chbkn.run/uploads/${file.filename}`);
            } else if(req.file){
    ...
    async update(req:req , res:res){
        ...
        if(req.files){
            if(Array.isArray(req.files) && req.files.length > 0){
                updateData.images = req.files.map((file:Express.Multer.File)=>`https://shophubapi.chbkn.run/uploads/${file.filename}`);
            }else if(req.file){
                updateData.images = [`https://shophubapi.chbkn.run/uploads/${req.file.filename}`];
            }
        };
